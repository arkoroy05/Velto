...
export class ContextGraphService {
   ...
   */
  async getContextGraph(projectId: ObjectId, userId: ObjectId): Promise<ContextGraph | null> {
    return await this.collection.findOne({ projectId, userId })
  }
   ...
   */
  async deleteContextGraph(projectId: ObjectId, userId: ObjectId): Promise<void> {
    await this.collection.deleteOne({ projectId, userId })
